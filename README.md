...
A production-grade, cloud-native e-commerce reference application built on **.NET 10** and **.NET Aspire 13.1**, demonstrating microservices architecture, Domain-Driven Design, CQRS, event-driven communication, and optional AI integration through OpenAI, Azure OpenAI, and Ollama.
...
> [!TIP]
> **Why This Matters**: The .NET Aspire orchestrator eliminates manual infrastructure configuration entirely — PostgreSQL, Redis, and RabbitMQ are provisioned automatically as containers and wired into every service with injected connection strings.

> [!IMPORTANT]
> **How It Works**: Aspire injects `ConnectionStrings__*` environment variables and service-discovery URLs into each process at startup. No edits to `appsettings.json` are required for local development.
...
Once the Aspire host is running, the full e-commerce storefront is accessible through the Aspire dashboard. Every service endpoint, structured log stream, and distributed trace is available from a single developer URL. The workflows below cover the most common day-to-day scenarios — browsing products, placing orders, running tests, and accessing API documentation.

All API services expose interactive OpenAPI documentation at their `/scalar` endpoint when running locally. Use the Aspire dashboard to resolve the dynamically assigned port for each service.

### Browsing the Catalog

Open the `webapp` link from the Aspire dashboard after running `dotnet run --project src/eShop.AppHost`. The storefront loads a paginated product grid. Filter by type or brand using the dropdowns, or enter a free-text query into the search box (AI semantic search must be enabled via `useOpenAI` or `useOllama` in `src/eShop.AppHost/Program.cs`).
...
Run end-to-end tests with Playwright (requires a running `AppHost`):
...
Running 3 tests using 3 workers
  ...
  ✓  e2e/BrowseItemTest.spec.ts (2.3s)
  ✓  e2e/AddItemTest.spec.ts (3.1s)
  ✓  e2e/RemoveItemTest.spec.ts (2.8s)
...
Nearly all eShop configuration is managed through the .NET Aspire `AppHost` via environment variables and connection strings injected at startup. Services use the standard `appsettings.json` plus environment-variable layering provided by `Microsoft.Extensions.Configuration`. No manual environment setup is required for local development — Aspire injects the correct `ConnectionStrings__*` and service-discovery URLs into each process automatically.

Advanced configurations (AI provider selection, identity token lifetimes, HTTPS vs HTTP launch profile) are controlled through flags in `src/eShop.AppHost/Program.cs` and `src/eShop.AppHost/Extensions.cs`. The table below covers the settings most likely to need adjustment for local customization or production deployment.

| Setting                                  | 📁 Location                         | ⚙️ Default               | 📝 Description                                            |
| ---------------------------------------- | ----------------------------------- | ------------------------ | --------------------------------------------------------- |
...
| 🤖 `ChatModelParameter`                  | Aspire parameter prompt             | `gpt-4.1-mini`           | Chat model name for AI assistant features                 |
| 🔍 `EmbeddingModelParameter`             | Aspire parameter prompt             | `text-embedding-3-small` | Text embedding model for semantic search                  |
...
| 🕐 `TokenLifetimeMinutes`                | `src/Identity.API/appsettings.json` | `120`                    | Standard identity token lifetime (minutes)                |
| 📅 `PermanentTokenLifetimeDays`          | `src/Identity.API/appsettings.json` | `365`                    | Permanent (remember-me) token lifetime (days)             |
...
| 🛒 Basket API        | Shopping cart read/write                          | gRPC 2.71, Redis               |
| 📋 Ordering API      | Order lifecycle management (CQRS + DDD)           | MediatR, FluentValidation      |
| 🔐 Identity API      | OAuth 2.0 / OpenID Connect provider               | Duende IdentityServer 7.3      |
