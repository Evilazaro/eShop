...
| ------------------------ | ---------------------------------------------------------------------------------- | --------------- | ------------- | ------------- | ---------- | ------------- | --------------------------- | ---------------------------------------- | ------------------------------------------------------------------------ |
| CatalogItem              | Product entity with pricing, stock levels, description, and 384-dim AI embedding   | Internal        | Relational DB | Catalog Team  | indefinite | real-time     | Catalog.API seed, Admin     | WebApp, ClientApp, Ordering domain       | src/Catalog.API/Model/CatalogItem.cs:7-70                                |
| CatalogBrand             | Brand reference entity for catalog product classification                          | Internal        | Relational DB | Catalog Team  | indefinite | batch         | catalog.json seed           | CatalogItem, WebApp, ClientApp           | src/Catalog.API/Model/CatalogBrand.cs:5-15                               |
| CatalogType              | Product category/type reference entity                                             | Internal        | Relational DB | Catalog Team  | indefinite | batch         | catalog.json seed           | CatalogItem, WebApp, ClientApp           | src/Catalog.API/Model/CatalogType.cs:5-15                                |
| Order                    | Order aggregate root with lifecycle status, address, and payment reference         | Financial       | Relational DB | Ordering Team | indefinite | real-time     | Ordering.API commands       | WebApp, OrderProcessor, PaymentProcessor | src/Ordering.Domain/AggregatesModel/OrderAggregate/Order.cs:6-80         |
| OrderItem                | Line item entity within an order; references product by ID with price snapshot     | Financial       | Relational DB | Ordering Team | indefinite | real-time     | Order aggregate             | Ordering.API, WebApp                     | src/Ordering.Domain/AggregatesModel/OrderAggregate/OrderItem.cs:6-40     |
| Buyer                    | Buyer aggregate root mapping identity GUID to domain buyer with payment methods    | PII             | Relational DB | Ordering Team | indefinite | real-time     | Identity.API (IdentityGuid) | Order aggregate, Ordering.API            | src/Ordering.Domain/AggregatesModel/BuyerAggregate/Buyer.cs:5-52         |
| PaymentMethod            | Payment method with card details (alias, card number, expiry, card type)           | PII + Financial | Relational DB | Ordering Team | indefinite | real-time     | Buyer aggregate             | Order aggregate                          | src/Ordering.Domain/AggregatesModel/BuyerAggregate/PaymentMethod.cs:5-40 |
| CardType                 | Enumerated card type reference (Visa, Mastercard, Amex, etc.)                      | Internal        | Relational DB | Ordering Team | indefinite | batch         | OrderingContextSeed         | PaymentMethod                            | src/Ordering.Domain/AggregatesModel/BuyerAggregate/CardType.cs:3-8       |
| ApplicationUser          | Identity user extending ASP.NET IdentityUser; includes card, address, and name PII | PII             | Relational DB | Identity Team | indefinite | real-time     | User registration           | All authenticated services (JWT claims)  | src/Identity.API/Models/ApplicationUser.cs:4-31                          |
| WebhookSubscription      | External webhook subscription with destination URL and event type                  | Internal        | Relational DB | Webhooks Team | indefinite | real-time     | Webhooks.API                | WebhooksSender                           | src/Webhooks.API/Model/WebhookSubscription.cs:3-14                       |
| CustomerBasket           | Basket aggregate stored in Redis; maps buyer ID to list of basket items            | Internal        | Key-Value     | Basket Team   | Session    | real-time     | WebApp, ClientApp (gRPC)    | Ordering.API (checkout)                  | src/Basket.API/Model/CustomerBasket.cs:3-15                              |
| BasketItem               | Line item in customer basket with product reference and quantity                   | Internal        | Key-Value     | Basket Team   | Session    | real-time     | WebApp, ClientApp (gRPC)    | CustomerBasket                           | src/Basket.API/Model/BasketItem.cs:3-25                                  |
| IntegrationEventLogEntry | Outbox entry storing serialized integration event; tracks publication state        | Internal        | Relational DB | Platform Team | 30d        | real-time     | All event publishers        | IntegrationEventLogService               | src/IntegrationEventLogEF/IntegrationEventLogEntry.cs:5-45               |
| ClientRequest            | Idempotency guard record for command deduplication (Id, Name, Time)                | Internal        | Relational DB | Ordering Team | 7d         | real-time     | Ordering.API commands       | RequestManager                           | src/Ordering.Infrastructure/Idempotency/ClientRequest.cs:4-10            |
| Address                  | Value object representing physical address; persisted as owned entity in Order     | PII             | Relational DB | Ordering Team | indefinite | real-time     | Order creation command      | Order entity, Ordering.API               | src/Ordering.Domain/AggregatesModel/OrderAggregate/Address.cs:5-22       |
...
| Component      | Description                                                                                            | Classification | Storage       | Owner         | Retention  | Freshness SLA | Source Systems                   | Consumers                    | Source File                                               |
| -------------- | ------------------------------------------------------------------------------------------------------ | -------------- | ------------- | ------------- | ---------- | ------------- | -------------------------------- | ---------------------------- | --------------------------------------------------------- |
| catalogdb      | PostgreSQL 16 database with pgvector extension for vector similarity search; image: ankane/pgvector    | Internal       | Relational DB | Catalog Team  | indefinite | real-time     | CatalogContext migrations        | Catalog.API                  | src/eShop.AppHost/Program.cs:10-15                        |
| orderingdb     | PostgreSQL 16 database for Ordering domain; uses `ordering` schema namespace                           | Financial      | Relational DB | Ordering Team | indefinite | real-time     | OrderingContext migrations       | Ordering.API, OrderProcessor | src/eShop.AppHost/Program.cs:10-17                        |
| identitydb     | PostgreSQL 16 database for ASP.NET Core Identity schema                                                | PII            | Relational DB | Identity Team | indefinite | real-time     | ApplicationDbContext migrations  | Identity.API                 | src/eShop.AppHost/Program.cs:10-16                        |
...
| Component                                              | Description                                                                                  | Classification | Storage        | Owner         | Retention | Freshness SLA | Source Systems    | Consumers                 | Source File                                                                                           |
| ------------------------------------------------------ | -------------------------------------------------------------------------------------------- | -------------- | -------------- | ------------- | --------- | ------------- | ----------------- | ------------------------- | ----------------------------------------------------------------------------------------------------- |
| OrderStartedIntegrationEvent                           | Published by Basket.API on checkout; triggers basket deletion and order creation             | Internal       | Message Broker | Basket Team   | real-time | real-time     | Basket.API        | Ordering.API              | src/Basket.API/IntegrationEvents/Events/OrderStartedIntegrationEvent.cs:6                             |
| ProductPriceChangedIntegrationEvent                    | Published by Catalog.API on price update; consumers update basket prices and trigger webhook | Internal       | Message Broker | Catalog Team  | real-time | real-time     | Catalog.API       | Basket.API, Webhooks.API  | src/Catalog.API/IntegrationEvents/Events/ProductPriceChangedIntegrationEvent.cs:6                     |
| OrderStatusChangedToAwaitingValidationIntegrationEvent | Published by Ordering.API; triggers Catalog to validate stock availability                   | Internal       | Message Broker | Ordering Team | real-time | real-time     | Ordering.API      | Catalog.API               | src/Catalog.API/IntegrationEvents/Events/OrderStatusChangedToAwaitingValidationIntegrationEvent.cs:3  |
| OrderStockConfirmedIntegrationEvent                    | Published by Catalog.API confirming stock is available for order                             | Internal       | Message Broker | Catalog Team  | real-time | real-time     | Catalog.API       | Ordering.API              | src/Catalog.API/IntegrationEvents/Events/OrderStockConfirmedIntegrationEvent.cs:3                     |
| OrderStockRejectedIntegrationEvent                     | Published by Catalog.API rejecting order due to insufficient stock                           | Internal       | Message Broker | Catalog Team  | real-time | real-time     | Catalog.API       | Ordering.API              | src/Catalog.API/IntegrationEvents/Events/OrderStockRejectedIntegrationEvent.cs:3                      |
| OrderStatusChangedToPaidIntegrationEvent               | Published by Ordering.API on payment; triggers Catalog stock decrement                       | Financial      | Message Broker | Ordering Team | real-time | real-time     | Ordering.API      | Catalog.API, Webhooks.API | src/Catalog.API/IntegrationEvents/Events/OrderStatusChangedToPaidIntegrationEvent.cs:3                |
| GracePeriodConfirmedIntegrationEvent                   | Published by OrderProcessor after grace period expiry; advances order to validation          | Internal       | Message Broker | Platform Team | real-time | real-time     | OrderProcessor    | Ordering.API              | src/Ordering.API/Application/IntegrationEvents/Events/GracePeriodConfirmedIntegrationEvent.cs:3       |
| OrderStatusChangedToStockConfirmedIntegrationEvent     | Published by Ordering.API; triggers PaymentProcessor                                         | Financial      | Message Broker | Ordering Team | real-time | real-time     | Ordering.API      | PaymentProcessor          | src/PaymentProcessor/IntegrationEvents/Events/OrderStatusChangedToStockConfirmedIntegrationEvent.cs:3 |
| OrderPaymentSucceededIntegrationEvent                  | Published by PaymentProcessor on successful payment                                          | Financial      | Message Broker | Platform Team | real-time | real-time     | PaymentProcessor  | Ordering.API              | src/PaymentProcessor/IntegrationEvents/Events/OrderPaymentSucceededIntegrationEvent.cs:3              |
| OrderPaymentFailedIntegrationEvent                     | Published by PaymentProcessor on payment failure                                             | Financial      | Message Broker | Platform Team | real-time | real-time     | PaymentProcessor  | Ordering.API              | src/PaymentProcessor/IntegrationEvents/Events/OrderPaymentFailedIntegrationEvent.cs:3                 |
...
| Component                                                | Description                                                                                                                           | Classification | Storage       | Owner         | Retention  | Freshness SLA | Source Systems                | Consumers                                                  | Source File                                                           |
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ------------- | ------------- | ---------- | ------------- | ----------------------------- | ---------------------------------------------------------- | --------------------------------------------------------------------- |
| IBasketRepository / RedisBasketRepository                | Repository interface and Redis implementation for CustomerBasket CRUD (GetBasketAsync, UpdateBasketAsync, DeleteBasketAsync)          | Internal       | Key-Value     | Basket Team   | Session    | real-time     | Redis                         | Basket.API gRPC, OrderStartedEventHandler                  | src/Basket.API/Repositories/RedisBasketRepository.cs:6-53             |
| IOrderRepository / OrderRepository                       | EF Core repository for Order aggregate access; implements IUnitOfWork                                                                 | Financial      | Relational DB | Ordering Team | indefinite | real-time     | orderingdb                    | Ordering.API command handlers                              | src/Ordering.Infrastructure/Repositories/OrderRepository.cs:4-34      |
| IBuyerRepository / BuyerRepository                       | EF Core repository for Buyer aggregate; includes PaymentMethods eager load; FindAsync by identity                                     | PII            | Relational DB | Ordering Team | indefinite | real-time     | orderingdb                    | DomainEventHandlers, Ordering.API                          | src/Ordering.Infrastructure/Repositories/BuyerRepository.cs:4-56      |
| IIntegrationEventLogService / IntegrationEventLogService | Outbox service: retrieves NotPublished events by transactionId, marks InProgress/Published                                            | Internal       | Relational DB | Platform Team | 30d        | real-time     | All event-publishing services | RabbitMQ publisher                                         | src/IntegrationEventLogEF/Services/IntegrationEventLogService.cs:5-35 |
...
| Component                              | Description                                                                                                                                          | Classification | Storage       | Owner            | Retention  | Freshness SLA | Source Systems            | Consumers                                             | Source File                                                            |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ------------- | ---------------- | ---------- | ------------- | ------------------------- | ----------------------------------------------------- | ---------------------------------------------------------------------- |
| Outbox Pattern (IntegrationEventLogEF) | IntegrationEventLogEntry stored in same DB transaction as domain changes; published asynchronously; state machine: NotPublished→InProgress→Published | Internal       | Relational DB | Platform Team    | 30d        | real-time     | Ordering.API, Catalog.API | IntegrationEventLogService, RabbitMQ publisher        | src/IntegrationEventLogEF/IntegrationEventLogEntry.cs:5-45             |
| EF Core Schema Migrations              | Version-controlled DDL changes for catalgoDb (3 migrations), orderingDb (4 migrations), identityDb (1), webhooksDb (1)                               | Internal       | Relational DB | All domain teams | indefinite | batch         | EF CLI, CI/CD pipeline    | All DbContexts                                        | src/Ordering.Infrastructure/Migrations/20230925222426_Initial.cs:1-100 |
...
| Component    | Description                                                                                                          | Classification | Storage       | Owner         | Retention  | Freshness SLA | Source Systems                                    | Consumers                                                       | Source File                                                            |
| ------------ | -------------------------------------------------------------------------------------------------------------------- | -------------- | ------------- | ------------- | ---------- | ------------- | ------------------------------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------- |
| CatalogBrand | Authoritative brand reference data seeded from catalog.json via CatalogContextSeed; idempotent seed-if-empty pattern | Internal       | Relational DB | Catalog Team  | indefinite | batch         | catalog.json (src/Catalog.API/Setup/catalog.json) | CatalogItem (FK), WebApp filter, ClientApp filter               | src/Catalog.API/Infrastructure/CatalogContextSeed.cs:28-32             |
| CatalogType  | Authoritative product type reference data seeded from catalog.json; distinct types extracted from seed file          | Internal       | Relational DB | Catalog Team  | indefinite | batch         | catalog.json (src/Catalog.API/Setup/catalog.json) | CatalogItem (FK), WebApp filter, ClientApp filter               | src/Catalog.API/Infrastructure/CatalogContextSeed.cs:34-38             |
...
| Component                           | Description                                                                                                                                                    | Classification | Storage       | Owner         | Retention  | Freshness SLA | Source Systems                              | Consumers                                              | Source File                                                                            |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ------------- | ------------- | ---------- | ------------- | ------------------------------------------- | ------------------------------------------------------ | -------------------------------------------------------------------------------------- |
| JSON Basket Serialization           | BasketSerializationContext (JsonSerializerContext) serializes/deserializes CustomerBasket to/from UTF-8 JSON bytes for Redis                                   | Internal       | Key-Value     | Basket Team   | Session    | real-time     | gRPC UpdateBasket requests                  | Redis database, GetBasket responses                    | src/Basket.API/Repositories/RedisBasketRepository.cs:31-36                             |
| IntegrationEvent JSON Serialization | IntegrationEvent serialized to/from JSON string Content via System.Text.Json for Outbox storage; deserialized by event type name                               | Internal       | Relational DB | Platform Team | 30d        | real-time     | Integration event publishers                | IntegrationEventLogService.DeserializeJsonContent      | src/IntegrationEventLogEF/IntegrationEventLogEntry.cs:38-44                            |
| Vector Embedding Transformation     | CatalogItem text (name + description) transformed to 384-dimensional float vector via IEmbeddingGenerator<string, Embedding<float>>; stored as pgvector column | Internal       | Relational DB | Catalog Team  | indefinite | batch         | AI model provider (Microsoft.Extensions.AI) | CatalogContext.CatalogItems.Embedding, semantic search | src/Catalog.API/Services/CatalogAI.cs:29-55                                            |
...
| Component                                     | Description                                                                                                                     | Classification | Storage        | Owner                           | Retention  | Freshness SLA | Source Systems                                 | Consumers                                      | Source File                                                                           |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | -------------- | -------------- | ------------------------------- | ---------- | ------------- | ---------------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------- |
| basket.proto                                  | gRPC Protocol Buffers v3 contract; defines Basket service with GetBasket, UpdateBasket, DeleteBasket RPCs and 5 message types   | Internal       | Not detected   | Basket Team                     | indefinite | real-time     | Basket.API (server), WebApp/ClientApp (client) | All gRPC basket consumers                      | src/Basket.API/Proto/basket.proto:1-40                                                |
| IntegrationEvent base record                  | Base record with Id (Guid) and CreationDate (DateTime); parent type for all 14 integration events in the system                 | Internal       | Message Broker | Platform Team                   | indefinite | real-time     | EventBus                                       | All integration event publishers and handlers  | src/EventBus/Events/IntegrationEvent.cs:3-17                                          |
| IBasketRepository contract                    | C# interface defining 3 data access operations (GetBasketAsync, UpdateBasketAsync, DeleteBasketAsync) with CustomerBasket model | Internal       | Key-Value      | Basket Team                     | indefinite | real-time     | RedisBasketRepository                          | BasketService (gRPC), OrderStartedEventHandler | src/Basket.API/Repositories/IBasketRepository.cs:5-10                                 |
...
| Component                                | Description                                                                                                                                       | Classification  | Storage       | Owner         | Retention    | Freshness SLA | Source Systems                             | Consumers                       | Source File                                                                                    |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- | ------------- | ------------- | ------------ | ------------- | ------------------------------------------ | ------------------------------- | ---------------------------------------------------------------------------------------------- |
| JWT Bearer Authentication                | JwtBearer middleware validates JWT tokens issued by Identity API; applied system-wide via ServiceDefaults.AddDefaultAuthentication                | Internal        | Not detected  | Platform Team | Session      | real-time     | Identity.API (token issuer)                | All protected API endpoints     | src/eShop.ServiceDefaults/AuthenticationExtensions.cs:33-52                                    |
| ASP.NET Core Identity (password hashing) | IdentityDbContext with standard Identity schema providing BCrypt password hashing, email confirmation, user claims                                | PII             | Relational DB | Identity Team | indefinite   | real-time     | ApplicationUser registration               | All authenticated API consumers | src/Identity.API/Data/ApplicationDbContext.cs:9-20                                             |
