...
```mermaid
---
title: eShop Data Classification Hierarchy
config:
  theme: base
  look: classic
  layout: dagre
...
---
flowchart TD
    accTitle: eShop Data Classification Hierarchy
    accDescr: Shows the five-tier data classification hierarchy from highest sensitivity PII + Financial down to Public, with required controls for each tier
    ...
    piiFinancial("🔒💰 PII + Financial\nPaymentMethod.CardNumber\nPCI-DSS controls required"):::danger
    pii("🔒 PII\nApplicationUser, Buyer\nAddress, CardHolderName"):::warning
    financial("💰 Financial\nOrder, OrderItem\nAudit + immutable records"):::warning
    internal("🔵 Internal\nCatalog, Basket, Events\nWebhooks, Idempotency"):::core
    public("🟢 Public\nCatalogBrand, CatalogType\nRead-only access"):::success

    piiFinancial -->|"highest sensitivity"| pii
    pii -->|"financial overlap"| financial
...
| 🏢 Component                         | 📝 Description                                                                                                         | 🏷️ Classification | 🗄️ Storage       | 👥 Owner      | ⏳ Retention | ⏱️ Freshness SLA | 👁️ Consumers                    |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- | ----------------- | ---------------- | ------------- | ------------ | ---------------- | ------------------------------- |
| CatalogContext                       | EF Core DbContext; DbSets: CatalogItems, CatalogBrands, CatalogTypes; pgvector extension registered                    | 🔵 Internal       | 🗄️ Relational DB | Catalog Team  | indefinite   | real-time        | Catalog.API, CatalogContextSeed |
| OrderingContext                      | EF Core DbContext implementing IUnitOfWork; DbSets: Orders, OrderItems, Payments, Buyers, CardTypes; schema: ordering  | 🔵 Internal       | 🗄️ Relational DB | Ordering Team | indefinite   | real-time        | Ordering.API, OrderProcessor    |
| ApplicationDbContext                 | EF Core IdentityDbContext for ApplicationUser; standard ASP.NET Core Identity schema                                   | 🔵 Internal       | 🗄️ Relational DB | Identity Team | indefinite   | real-time        | Identity.API, all JWT consumers |
| WebhooksContext                      | EF Core DbContext; DbSet: Subscriptions; indexed on UserId and Type                                                    | 🔵 Internal       | 🗄️ Relational DB | Webhooks Team | indefinite   | real-time        | WebhooksSender                  |
| basket.proto                         | Protocol Buffers v3 gRPC schema; defines Basket service, CustomerBasketResponse, BasketItem, request/response messages | 🔵 Internal       | ❓ Not detected  | Basket Team   | indefinite   | real-time        | Basket.API gRPC server          |
| CatalogItemEntityTypeConfiguration   | EF Fluent API: table=Catalog, vector(384) column, Name HasMaxLength(50), indexes on Name                               | 🔵 Internal       | 🗄️ Relational DB | Catalog Team  | indefinite   | real-time        | EF Core migration engine        |
| CatalogBrandEntityTypeConfiguration  | EF Fluent API: standard brand table configuration                                                                      | 🔵 Internal       | 🗄️ Relational DB | Catalog Team  | indefinite   | real-time        | EF Core migration engine        |
| CatalogTypeEntityTypeConfiguration   | EF Fluent API: standard type table configuration                                                                       | 🔵 Internal       | 🗄️ Relational DB | Catalog Team  | indefinite   | real-time        | EF Core migration engine        |
| OrderEntityTypeConfiguration         | EF Fluent API: table=orders, HiLo(orderseq), OwnsOne Address, PaymentId→PaymentMethodId column, FK constraints         | 🔵 Internal       | 🗄️ Relational DB | Ordering Team | indefinite   | real-time        | EF Core migration engine        |
| OrderItemEntityTypeConfiguration     | EF Fluent API: table=orderitems, HiLo(orderitemseq)                                                                    | 🔵 Internal       | 🗄️ Relational DB | Ordering Team | indefinite   | real-time        | EF Core migration engine        |
| PaymentMethodEntityTypeConfiguration | EF Fluent API: table=paymentmethods, HiLo(paymentseq), private field mapping for CardHolderName, Alias, CardNumber     | 🔵 Internal       | 🗄️ Relational DB | Ordering Team | indefinite   | real-time        | EF Core migration engine        |
...
| 🏢 Component                     | 📝 Description                                                                                                                               | 🏷️ Classification | 🗄️ Storage       | 👥 Owner      | ⏳ Retention | ⏱️ Freshness SLA | 👁️ Consumers                                          |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ---------------- | ------------- | ------------ | ---------------- | ----------------------------------------------------- |
| [Required] attribute enforcement | Applied to Name (CatalogItem, CatalogBrand, CatalogType), EventTypeName (IntegrationEventLogEntry), Content, DestUrl, UserId, CardHolderName | 🔵 Internal       | ❓ Not detected  | Platform Team | Not detected | Not detected     | EF Core model validation, ASP.NET validation pipeline |
| HasMaxLength constraints         | String length caps: Name 50 (CatalogItem), CardHolderName 200, Alias 200, CardNumber 25, OrderStatus 30, IdentityGuid 200                    | 🔵 Internal       | 🗄️ Relational DB | Platform Team | Not detected | Not detected     | EF Core, PostgreSQL schema                            |
| BasketItem quantity validation   | IValidatableObject enforces Quantity ≥ 1; returns ValidationResult if violated                                                               | 🔵 Internal       | ❓ Not detected  | Basket Team   | Not detected | Not detected     | BasketService.UpdateBasket                            |
| OrderItem business invariants    | Units > 0; (unitPrice × units) ≥ discount; throws OrderingDomainException on violation                                                       | 💰 Financial      | ❓ Not detected  | Ordering Team | Not detected | Not detected     | Order.AddOrderItem                                    |
| Order domain exception guards    | Null/whitespace validation on all Order constructor parameters; card expiration must be in future                                            | 💰 Financial      | ❓ Not detected  | Ordering Team | Not detected | Not detected     | Order constructor                                     |
| Buyer identity guard             | IdentityGuid and Name must be non-null/whitespace; throws ArgumentNullException                                                              | 🔒 PII            | ❓ Not detected  | Ordering Team | Not detected | Not detected     | Buyer constructor                                     |
| IntegrationEvent state machine   | EventStateEnum (NotPublished, InProgress, Published, PublishedFailed); state transitions tracked in database                                 | 🔵 Internal       | 🗄️ Relational DB | Platform Team | 30d          | real-time        | RabbitMQ publisher, retry logic                       |
| CatalogItem stock invariants     | RemoveStock validates quantityDesired > 0 and available stock; AddStock validates quantityAdded > 0                                          | 🔵 Internal       | 🗄️ Relational DB | Catalog Team  | Not detected | real-time        | Catalog.API, Ordering event handlers                  |
...
| All 11 component type subsections (5.1-5.11)              | ✅ PASS | All 11 subsections present with 8-column table schema (Source Systems + Source File removed) |
| Section 2 table schema (3 columns)                        | ✅ PASS | Name, Description, Classification in all 2.x tables                 |
| Section 5 table schema (8 columns)                        | ✅ PASS | Source Systems + Source File columns removed; 8 mandatory columns in all 5.x tables |
| Evidence column removed from 6 tables                     | ✅ PASS | Sections 1, 3, 4, 7 tables updated                                  |
| Quick TOC added                                           | ✅ PASS | Jump links to all 9 sections at document top                        |
| data_layer_reasoning YAML block present                   | ✅ PASS | YAML block included immediately before Section 1 heading            |
| Emoji headings on all sections and subsections            | ✅ PASS | All ## and ### headings have contextual emoji prefixes               |
| Emoji column headers on all tables                        | ✅ PASS | All table headers across all sections updated                       |
| Emoji cell values in Classification columns               | ✅ PASS | 🔵💰🔒🔒💰🟢 prefixes applied to all Classification cells         |
| Classification Taxonomy Mermaid diagram                   | ✅ PASS | Hierarchy flowchart added in Section 3 after taxonomy table         |
