...
| Name                                      | Description                                                                                                                                                               | Source                                                                                                                   | Confidence  | Maturity          |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ----------- | ----------------- |
| Order Placement Process                   | Customer-initiated process of providing order details, delivery address, and payment information to create a new order in Submitted state and trigger basket clearance.   | `src/Ordering.API/Application/Commands/CreateOrderCommandHandler.cs:1-65`                                                | 0.85 (HIGH) | Level 3 – Defined |
| Order Grace Period & Validation Process   | Background process that observes all submitted orders for a configured duration (grace period), then confirms eligibility for stock validation.                           | `src/OrderProcessor/Services/GracePeriodManagerService.cs:1-60`                                                          | 0.82 (HIGH) | Level 3 – Defined |
| Stock Validation Process                  | Coordinated process checking available stock for all items in an awaiting-validation order, resulting in confirmation or rejection and corresponding stock adjustment.    | `src/Catalog.API/IntegrationEvents/Events/OrderStatusChangedToAwaitingValidationIntegrationEvent.cs:1-10`                | 0.81 (HIGH) | Level 3 – Defined |
| Payment Execution Process                 | Process triggered when an order's stock has been confirmed; evaluates payment method and records a payment success or failure outcome, advancing or cancelling the order. | `src/PaymentProcessor/IntegrationEvents/EventHandling/OrderStatusChangedToStockConfirmedIntegrationEventHandler.cs:1-35` | 0.83 (HIGH) | Level 3 – Defined |
| Buyer Registration & Verification Process | Process that validates or creates a buyer profile and verifies/adds a payment method when an order is first started, ensuring customer identity integrity across orders.  | `src/Ordering.API/Application/DomainEventHandlers/ValidateOrAddBuyerAggregateWhenOrderStartedDomainEventHandler.cs:1-55` | 0.85 (HIGH) | Level 3 – Defined |

### 2.5 Business Services (6)

| Name                         | Description                                                                                                                                                                    | Source                                                                                                  | Confidence  | Maturity             |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- | ----------- | -------------------- |
| Ordering Service             | Business service that manages the full order lifecycle — accepting order creation, enforcing state transitions, coordinating domain events, and publishing integration events. | `src/Ordering.API/Program.cs:1-30`, `src/Ordering.Domain/AggregatesModel/OrderAggregate/Order.cs:1-160` | 0.90 (HIGH) | Level 4 – Measured   |
| Basket Service               | Business service that maintains each customer's active shopping basket, providing retrieval, update, and deletion operations via a gRPC interface.                             | `src/Basket.API/Program.cs:1-20`, `src/WebApp/Services/BasketService.cs:1-50`                           | 0.85 (HIGH) | Level 3 – Defined    |
| Catalog Service              | Business service that provides the product catalog — including item details, stock levels, brands, types, and search — to shoppers and administrators.                         | `src/Catalog.API/Program.cs:1-20`, `src/Catalog.API/Model/CatalogItem.cs:1-62`                          | 0.87 (HIGH) | Level 3 – Defined    |
| Payment Processor Service    | Business service that receives stock-confirmed order events and simulates payment processing, publishing success or failure outcomes to the event bus.                         | `src/PaymentProcessor/Program.cs:1-20`, `src/PaymentProcessor/PaymentOptions.cs:1-10`                   | 0.82 (HIGH) | Level 2 – Repeatable |
...
| Name                          | Description                                                                                                                                                                             | Source                                                                          | Confidence  | Maturity           |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | ----------- | ------------------ |
| Address Completeness Rule     | All orders must include a complete delivery address: street, city, state, country, and zip code — none may be empty.                                                                    | `src/Ordering.API/Application/Validations/CreateOrderCommandValidator.cs:5-7`   | 0.90 (HIGH) | Level 3 – Defined  |
| Card Number Length Rule       | Payment card number must be between 12 and 19 characters in length.                                                                                                                     | `src/Ordering.API/Application/Validations/CreateOrderCommandValidator.cs:8`     | 0.90 (HIGH) | Level 3 – Defined  |
| Card Holder Name Rule         | Payment card holder name must not be empty.                                                                                                                                             | `src/Ordering.API/Application/Validations/CreateOrderCommandValidator.cs:9`     | 0.89 (HIGH) | Level 3 – Defined  |
| Card Expiration Rule          | Payment card expiration date must be a future date (≥ current UTC time).                                                                                                                | `src/Ordering.API/Application/Validations/CreateOrderCommandValidator.cs:10-12` | 0.91 (HIGH) | Level 3 – Defined  |
| Security Number Length Rule   | Payment security number (CVV) must be exactly 3 characters.                                                                                                                             | `src/Ordering.API/Application/Validations/CreateOrderCommandValidator.cs:13`    | 0.90 (HIGH) | Level 3 – Defined  |
| Minimum Order Items Rule      | Each order must contain at least one order item; empty cart checkout is not permitted.                                                                                                  | `src/Ordering.API/Application/Validations/CreateOrderCommandValidator.cs:14-16` | 0.91 (HIGH) | Level 3 – Defined  |
| Order State Transition Rule   | Order status follows a strict, forward-only lifecycle sequence: Submitted → AwaitingValidation → StockConfirmed → Paid → Shipped, with Cancelled reachable only before Paid or Shipped. | `src/Ordering.Domain/AggregatesModel/OrderAggregate/Order.cs:89-140`            | 0.93 (HIGH) | Level 4 – Measured |
| Cancellation Eligibility Rule | An order may only be cancelled before it reaches the Paid or Shipped status; attempts after those states raise a domain exception.                                                      | `src/Ordering.Domain/AggregatesModel/OrderAggregate/Order.cs:123-145`           | 0.92 (HIGH) | Level 4 – Measured |
| Discount Maximization Rule    | When an item is added to an existing order line, only a higher discount replaces the current one; a lower discount offer is silently ignored.                                           | `src/Ordering.Domain/AggregatesModel/OrderAggregate/Order.cs:69-85`             | 0.88 (HIGH) | Level 3 – Defined  |
| Stock Reorder Threshold Rule  | A catalog item's available stock may not be reduced below zero; if remaining stock after a transaction falls at or below the RestockThreshold, a restock alert is triggered.            | `src/Catalog.API/Model/CatalogItem.cs:50-75`                                    | 0.85 (HIGH) | Level 3 – Defined  |
| Order Idempotency Rule        | Duplicate order creation requests bearing the same idempotency key must be silently accepted and return success without creating duplicate orders.                                      | `src/Ordering.API/Application/Commands/CreateOrderCommandHandler.cs:60-70`      | 0.86 (HIGH) | Level 3 – Defined  |

### 2.9 Business Events (14)

| Name                                              | Description                                                                                                 | Source                                                                                           | Confidence  | Maturity          |
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ----------- | ----------------- |
| Order Started Domain Event                        | Raised when a new order is created; triggers buyer verification and basket clearance.                       | `src/Ordering.Domain/Events/OrderStartedDomainEvent.cs`                                          | 0.91 (HIGH) | Level 3 – Defined |
| Buyer Payment Method Verified Domain Event        | Raised when a buyer's payment method has been verified or added; triggers order status update to Submitted. | `src/Ordering.Domain/Events/BuyerPaymentMethodVerifiedDomainEvent.cs`                            | 0.88 (HIGH) | Level 3 – Defined |
| Order Status Changed to Awaiting Validation Event | Raised when the order grace period ends and the order advances to stock validation.                         | `src/Ordering.Domain/Events/OrderStatusChangedToAwaitingValidationDomainEvent.cs`                | 0.90 (HIGH) | Level 3 – Defined |
| Order Status Changed to Stock Confirmed Event     | Raised when all ordered items have confirmed stock; triggers payment execution.                             | `src/Ordering.Domain/Events/OrderStatusChangedToStockConfirmedDomainEvent.cs`                    | 0.90 (HIGH) | Level 3 – Defined |
| Order Status Changed to Paid Event                | Raised when payment succeeds; advances order to Paid state.                                                 | `src/Ordering.Domain/Events/OrderStatusChangedToPaidDomainEvent.cs`                              | 0.90 (HIGH) | Level 3 – Defined |
| Order Shipped Domain Event                        | Raised when an order has been dispatched to the customer.                                                   | `src/Ordering.Domain/Events/OrderShippedDomainEvent.cs`                                          | 0.91 (HIGH) | Level 3 – Defined |
| Order Cancelled Domain Event                      | Raised when an order is cancelled; initiates compensating actions.                                          | `src/Ordering.Domain/Events/OrderCancelledDomainEvent.cs`                                        | 0.91 (HIGH) | Level 3 – Defined |
| Grace Period Confirmed Integration Event          | Signals that an order's grace period has elapsed and it is eligible for stock validation.                   | `src/OrderProcessor/Events/GracePeriodConfirmedIntegrationEvent.cs`                              | 0.87 (HIGH) | Level 3 – Defined |
| Order Started Integration Event                   | Cross-service event notifying the basket service to clear the customer's basket after order placement.      | `src/Basket.API/IntegrationEvents/Events/OrderStartedIntegrationEvent.cs`                        | 0.87 (HIGH) | Level 3 – Defined |
| Order Stock Confirmed Integration Event           | Cross-service event confirming that all order items have available stock.                                   | `src/Catalog.API/IntegrationEvents/Events/OrderStockConfirmedIntegrationEvent.cs`                | 0.88 (HIGH) | Level 3 – Defined |
| Order Stock Rejected Integration Event            | Cross-service event indicating one or more order items lack sufficient stock; triggers order cancellation.  | `src/Catalog.API/IntegrationEvents/Events/OrderStockRejectedIntegrationEvent.cs`                 | 0.88 (HIGH) | Level 3 – Defined |
| Order Payment Succeeded Integration Event         | Cross-service event confirming successful payment for a stock-confirmed order.                              | `src/Ordering.API/Application/IntegrationEvents/Events/OrderPaymentSucceededIntegrationEvent.cs` | 0.88 (HIGH) | Level 3 – Defined |
| Order Payment Failed Integration Event            | Cross-service event indicating payment failure; triggers order cancellation.                                | `src/Ordering.API/Application/IntegrationEvents/Events/OrderPaymentFailedIntegrationEvent .cs`   | 0.88 (HIGH) | Level 3 – Defined |
| Product Price Changed Integration Event           | Cross-service event notifying registered subscribers of a catalog item price change via webhook.            | `src/Catalog.API/IntegrationEvents/Events/ProductPriceChangedIntegrationEvent.cs`                | 0.84 (HIGH) | Level 3 – Defined |

### 2.10 Business Objects/Entities (10)

| Name                | Description                                                                                                                                                                           | Source                                                                     | Confidence  | Maturity           |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ----------- | ------------------ |
| Order               | Core business entity and aggregate root representing a customer's purchase commitment; holds items, delivery address, status, and buyer reference.                                    | `src/Ordering.Domain/AggregatesModel/OrderAggregate/Order.cs:1-160`        | 0.93 (HIGH) | Level 4 – Measured |
| OrderItem           | Line-item component of an order, capturing product identity, quantity, unit price, and applicable discount.                                                                           | `src/Ordering.Domain/AggregatesModel/OrderAggregate/OrderItem.cs`          | 0.88 (HIGH) | Level 3 – Defined  |
| OrderStatus         | Enumerated business state domain object defining all legal lifecycle states for an order: Submitted, AwaitingValidation, StockConfirmed, Paid, Shipped, Cancelled.                    | `src/Ordering.Domain/AggregatesModel/OrderAggregate/OrderStatus.cs:5-14`   | 0.91 (HIGH) | Level 4 – Measured |
| Buyer               | Business entity representing a customer in the ordering context; holds identity reference and a collection of verified payment methods.                                               | `src/Ordering.Domain/AggregatesModel/BuyerAggregate/Buyer.cs:1-55`         | 0.88 (HIGH) | Level 3 – Defined  |
| PaymentMethod       | Business entity recording a buyer's registered payment instrument — card type, masked card number, holder name, and expiration — used for order payment.                              | `src/Ordering.Domain/AggregatesModel/BuyerAggregate/PaymentMethod.cs:1-45` | 0.86 (HIGH) | Level 3 – Defined  |
| Address             | Value object capturing a delivery location: street, city, state, country, and zip code. Enforces structural equality by field composition.                                            | `src/Ordering.Domain/AggregatesModel/OrderAggregate/Address.cs:1-32`       | 0.87 (HIGH) | Level 3 – Defined  |
| CatalogItem         | Business entity representing a product in the catalog: name, description, price, brand, type, stock levels, and reorder thresholds.                                                   | `src/Catalog.API/Model/CatalogItem.cs:1-62`                                | 0.86 (HIGH) | Level 3 – Defined  |
| CustomerBasket      | Business entity representing a customer's active pre-order selection; identified by buyer ID and containing a list of product quantity pairs.                                         | `src/Basket.API/Model/CustomerBasket.cs:1-20`                              | 0.83 (HIGH) | Level 3 – Defined  |
| WebhookSubscription | Business entity recording a subscriber's registration to receive event notifications at a specified destination URL, scoped to a webhook type and user identity.                      | `src/Webhooks.API/Model/WebhookSubscription.cs:1-15`                       | 0.81 (HIGH) | Level 3 – Defined  |
| ApplicationUser     | Business entity representing a registered customer's full profile including contact details (address, city, state, country, zip), payment card information, and identity credentials. | `src/Identity.API/Models/ApplicationUser.cs:1-30`                          | 0.82 (HIGH) | Level 3 – Defined  |

### 2.11 KPIs & Metrics (4)

| Name                              | Description                                                                                                                                                               | Source                                                                   | Confidence    | Maturity             |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | ------------- | -------------------- |
| Order Status Distribution         | Observable metric tracking how many orders reside in each of the six lifecycle states at any point in time; derived from the OrderStatus state-machine instrumentation.   | `src/Ordering.Domain/AggregatesModel/OrderAggregate/OrderStatus.cs:5-14` | 0.78 (MEDIUM) | Level 2 – Repeatable |
| Stock Level vs. Reorder Threshold | Operational metric comparing a catalog item's AvailableStock against its RestockThreshold to signal reorder necessity; MaxStockThreshold defines upper inventory bound.   | `src/Catalog.API/Model/CatalogItem.cs:25-35`                             | 0.77 (MEDIUM) | Level 2 – Repeatable |
| Grace Period Duration             | Configurable timing metric defining how long an order remains in the submitted state before advancing to validation; controlled by BackgroundTaskOptions.CheckUpdateTime. | `src/OrderProcessor/BackgroundTaskOptions.cs:1-10`                       | 0.73 (MEDIUM) | Level 2 – Repeatable |
| Payment Outcome Rate              | Configurable metric expressing the simulated probability of payment success vs. failure; expressed as a binary configurable flag (PaymentOptions.PaymentSucceeded).       | `src/PaymentProcessor/PaymentOptions.cs:1-10`                            | 0.73 (MEDIUM) | Level 2 – Repeatable |
...
| ---------------------------------- | -------------------- | ---------------------------------------------------------------- | ---------------------------------------- |
| Order Lifecycle Management         | Level 4 – Measured   | 6-state machine, 11 business rules, 14 events, CQRS, idempotency | Formal SLA metrics and reporting         |
| Product Catalog Management         | Level 3 – Defined    | CatalogItem model, stock thresholds, price change events         | KPI dashboards, formal admin audit trail |
| Shopping Basket Management         | Level 3 – Defined    | CustomerBasket aggregate, gRPC API, event-driven clearance       | Basket abandonment tracking              |
| Customer Identity & Authentication | Level 3 – Defined    | ApplicationUser model, OAuth2 integration, profile completeness  | Multi-factor auth policy documentation   |
| Payment Processing                 | Level 2 – Repeatable | Configurable success simulation, event publication               | Real gateway integration, retry policies |
| Event-Driven Notification          | Level 2 – Repeatable | Webhook subscription model, typed events, URL delivery           | Delivery retry, failure tracking, SLAs   |
...
| **Source**      | `README.md:1-30`, `azure.yaml:1-20`, `eShop.slnx:1-30`                                                                                                                                                                                                                                                                                                                                                                 |
| **Confidence**  | 0.74 (MEDIUM)                                                                                                                                                                                                                                                                                                                                                                                                          |
| **Maturity**    | Level 2 – Repeatable — strategy observable but not formally documented                                                                                                                                                                                                                                                                                                                                                 |
...
| **Source**      | `src/Catalog.API/Model/CatalogItem.cs:1-62`, `src/Catalog.API/Model/CatalogBrand.cs`, `src/Catalog.API/Model/CatalogType.cs`                                                                                                                                                                                                       |
...
| **Source**      | `src/Basket.API/Model/CustomerBasket.cs:1-20`, `src/Basket.API/Model/BasketItem.cs`                                                                                                                                                                                                                        |
...
| **Source**      | `src/Ordering.Domain/AggregatesModel/OrderAggregate/Order.cs:1-160`, `src/Ordering.Domain/AggregatesModel/OrderAggregate/OrderStatus.cs:1-14`                                                                                                                                                                                                                         |
...
| **Source**      | `src/PaymentProcessor/IntegrationEvents/EventHandling/OrderStatusChangedToStockConfirmedIntegrationEventHandler.cs:1-35`, `src/PaymentProcessor/PaymentOptions.cs:1-10`                                                                                                                                                         |
...
| **Source**      | `src/Identity.API/Models/ApplicationUser.cs:1-30`, `src/Identity.API/Program.cs:1-20`                                                                                                                                                                                 |
...
| **Source**      | `src/Webhooks.API/Model/WebhookSubscription.cs:1-15`, `src/Webhooks.API/Model/WebhookType.cs:1-8`, `src/Webhooks.API/Services/WebhooksSender.cs:1-30`                                                                                                           |
...
| **Source**      | `src/WebApp/Services/BasketService.cs:1-50`, `src/WebApp/Services/OrderingService.cs:1-20`, `src/WebApp/Services/BasketCheckoutInfo.cs:1-40`                                                                                                                                                                   |
| **Confidence**  | 0.82 (HIGH)                                                                                                                                                                                                                                                                                                    |
| **Maturity**    | Level 3 – Defined                                                                                                                                                                                                                                                                                              |
...
| **Source**      | `src/Catalog.API/Model/CatalogItem.cs:1-62`, `src/Catalog.API/IntegrationEvents/Events/ProductPriceChangedIntegrationEvent.cs`                                                                                                                                                                   |
...
| **Source**      | `src/Ordering.API/Application/Commands/CreateOrderCommandHandler.cs:1-65`, `src/Ordering.API/Application/Validations/CreateOrderCommandValidator.cs:1-30`                                                                                                                                                                          |
| **Confidence**  | 0.85 (HIGH)                                                                                                                                                                                                                                                                                                                        |
| **Maturity**    | Level 3 – Defined                                                                                                                                                                                                                                                                                                                  |
...
| **Source**      | `src/OrderProcessor/Services/GracePeriodManagerService.cs:1-65`, `src/OrderProcessor/BackgroundTaskOptions.cs:1-10`                                                                                                                                                                                                                  |
| **Confidence**  | 0.82 (HIGH)                                                                                                                                                                                                                                                                                                                          |
| **Maturity**    | Level 3 – Defined                                                                                                                                                                                                                                                                                                                    |
...
| **Source**      | `src/Catalog.API/IntegrationEvents/Events/OrderStatusChangedToAwaitingValidationIntegrationEvent.cs`, `src/Catalog.API/IntegrationEvents/Events/OrderStockConfirmedIntegrationEvent.cs`                                                                                                                                       |
...
| **Source**      | `src/PaymentProcessor/IntegrationEvents/EventHandling/OrderStatusChangedToStockConfirmedIntegrationEventHandler.cs:1-35`                                                                                                                                                                                                |
| **Confidence**  | 0.83 (HIGH)                                                                                                                                                                                                                                                                                                             |
| **Maturity**    | Level 2 – Repeatable                                                                                                                                                                                                                                                                                                    |
...
| **Source**      | `src/Ordering.API/Application/DomainEventHandlers/ValidateOrAddBuyerAggregateWhenOrderStartedDomainEventHandler.cs:1-55`                                                                                                                                                                                                                        |
| **Confidence**  | 0.85 (HIGH)                                                                                                                                                                                                                                                                                                                                     |
| **Maturity**    | Level 3 – Defined                                                                                                                                                                                                                                                                                                                               |
...
| **Source**      | `src/Ordering.API/Program.cs`, `src/Ordering.Domain/AggregatesModel/OrderAggregate/Order.cs:1-160`                                                                                                                                                                                                                |
...
| **Source**      | `src/Basket.API/Program.cs`, `src/WebApp/Services/BasketService.cs:1-50`                                                                                                                                               |
| **Confidence**  | 0.85 (HIGH)                                                                                                                                                                                                            |
| **Maturity**    | Level 3 – Defined                                                                                                                                                                                                      |
...
| **Source**      | `src/Catalog.API/Program.cs`, `src/Catalog.API/Model/CatalogItem.cs:1-62`                                                                                                                                                                                                      |
...
| **Source**      | `src/PaymentProcessor/Program.cs`, `src/PaymentProcessor/PaymentOptions.cs:1-10`                                                                                                                                                                                            |
| **Confidence**  | 0.82 (HIGH)                                                                                                                                                                                                                                                                 |
| **Maturity**    | Level 2 – Repeatable                                                                                                                                                                                                                                                        |
...
| **Source**      | `src/Identity.API/Program.cs`, `src/Identity.API/Models/ApplicationUser.cs:1-30`                                                                                                                                                                              |
...
| **Source**      | `src/Webhooks.API/Program.cs`, `src/Webhooks.API/Services/WebhooksSender.cs:1-30`, `src/Webhooks.API/Services/GrantUrlTesterService.cs`                                                                                                                       |
| **Confidence**  | 0.80 (HIGH)                                                                                                                                                                                                                                                   |
| **Maturity**    | Level 3 – Defined                                                                                                                                                                                                                                             |
...
| **Source**      | `src/Ordering.API/Application/Commands/`, `src/Ordering.API/Application/DomainEventHandlers/`                                                                                                                                                           |
...
| **Source**      | `src/Identity.API/Quickstart/`, `src/Identity.API/Models/`                                                                                                                                                |
| **Confidence**  | 0.78 (HIGH)                                                                                                                                                                                               |
| **Maturity**    | Level 2 – Repeatable                                                                                                                                                                                      |
...
| **Source**      | `src/PaymentProcessor/IntegrationEvents/`                                                                                                                                                                             |
| **Confidence**  | 0.82 (HIGH)                                                                                                                                                                                                           |
| **Maturity**    | Level 2 – Repeatable                                                                                                                                                                                                  |
...
| **Source**      | `src/Webhooks.API/Services/`, `src/Webhooks.API/Apis/`                                                                                                                                                                               |
| **Confidence**  | 0.82 (HIGH)                                                                                                                                                                                                                          |
| **Maturity**    | Level 2 – Repeatable                                                                                                                                                                                                                 |
...
| **Source**      | `src/PaymentProcessor/PaymentOptions.cs:1-10`                                                                                                                                                                                                                             |
...
| **Source**      | `src/Ordering.API/Application/Validations/CreateOrderCommandValidator.cs:5-7`                                                                                                            |
| **Confidence**  | 0.90 (HIGH)                                                                                                                                                                              |
| **Maturity**    | Level 3 – Defined                                                                                                                                                                        |
...
| **Source**      | `src/Ordering.API/Application/Validations/CreateOrderCommandValidator.cs:8`                                                                                                   |
| **Confidence**  | 0.90 (HIGH)                                                                                                                                                                   |
| **Maturity**    | Level 3 – Defined                                                                                                                                                             |
...
| **Source**      | `src/Ordering.API/Application/Validations/CreateOrderCommandValidator.cs:10-12`                                                                                                                                                   |
...
| **Source**      | `src/Ordering.API/Application/Validations/CreateOrderCommandValidator.cs:13`                                                                      |
| **Confidence**  | 0.90 (HIGH)                                                                                                                                       |
| **Maturity**    | Level 3 – Defined                                                                                                                                 |
...
| **Source**      | `src/Ordering.API/Application/Validations/CreateOrderCommandValidator.cs:14-16`                                                                       |
| **Confidence**  | 0.91 (HIGH)                                                                                                                                           |
| **Maturity**    | Level 3 – Defined                                                                                                                                     |
...
| **Source**      | `src/Ordering.Domain/AggregatesModel/OrderAggregate/Order.cs:123-145`                                                                                                                                              |
...
| **Source**      | `src/Ordering.Domain/AggregatesModel/OrderAggregate/Order.cs:69-85`                                                                                                                                                                                               |
| **Confidence**  | 0.88 (HIGH)                                                                                                                                                                                                                                                       |
| **Maturity**    | Level 3 – Defined                                                                                                                                                                                                                                                 |
...
| **Source**      | `src/Ordering.API/Application/Validations/CreateOrderCommandValidator.cs:9`                                                                        |
...
| **Source**      | `src/Catalog.API/Model/CatalogItem.cs:50-75`                                                                                                                                                                                                  |
| **Confidence**  | 0.85 (HIGH)                                                                                                                                                                                                                                   |
| **Maturity**    | Level 3 – Defined                                                                                                                                                                                                                             |
...
| **Source**      | `src/Ordering.API/Application/Commands/CreateOrderCommandHandler.cs:60-70`                                                                                                                                                                                       |
| **Confidence**  | 0.86 (HIGH)                                                                                                                                                                                                                                                      |
| **Maturity**    | Level 3 – Defined                                                                                                                                                                                                                                                |
...
| #   | Name                                | Tier        | Trigger                                 | Effect                                    | Source                                                                                           | Confidence |
| --- | ----------------------------------- | ----------- | --------------------------------------- | ----------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------- |
| 1   | Order Started Domain Event          | Domain      | Order placement                         | Buyer registration, basket clearance      | `src/Ordering.Domain/Events/OrderStartedDomainEvent.cs`                                          | 0.91       |
| 2   | Buyer Payment Method Verified       | Domain      | Buyer verification handler              | Order submission event published          | `src/Ordering.Domain/Events/BuyerPaymentMethodVerifiedDomainEvent.cs`                            | 0.88       |
| 3   | awaiting Validation Status Changed  | Domain      | Grace period confirmed                  | Order advances to AwaitingValidation      | `src/Ordering.Domain/Events/OrderStatusChangedToAwaitingValidationDomainEvent.cs`                | 0.90       |
| 4   | Stock Confirmed Status Changed      | Domain      | Stock validation event                  | Order advances to StockConfirmed          | `src/Ordering.Domain/Events/OrderStatusChangedToStockConfirmedDomainEvent.cs`                    | 0.90       |
| 5   | Paid Status Changed                 | Domain      | Payment success event                   | Order advances to Paid                    | `src/Ordering.Domain/Events/OrderStatusChangedToPaidDomainEvent.cs`                              | 0.90       |
| 6   | Order Shipped Domain Event          | Domain      | Ship order command                      | Order marked Shipped; webhooks triggered  | `src/Ordering.Domain/Events/OrderShippedDomainEvent.cs`                                          | 0.91       |
| 7   | Order Cancelled Domain Event        | Domain      | Cancel command or stock/payment failure | Order marked Cancelled                    | `src/Ordering.Domain/Events/OrderCancelledDomainEvent.cs`                                        | 0.91       |
| 8   | Grace Period Confirmed Integration  | Integration | GracePeriodManagerService timer         | Order advances to AwaitingValidation      | `src/OrderProcessor/Events/GracePeriodConfirmedIntegrationEvent.cs`                              | 0.87       |
| 9   | Order Started Integration Event     | Integration | Order placement                         | Basket service clears customer basket     | `src/Basket.API/IntegrationEvents/Events/OrderStartedIntegrationEvent.cs`                        | 0.87       |
| 10  | Order Stock Confirmed Integration   | Integration | Catalog stock validation                | Ordering advances order to StockConfirmed | `src/Catalog.API/IntegrationEvents/Events/OrderStockConfirmedIntegrationEvent.cs`                | 0.88       |
| 11  | Order Stock Rejected Integration    | Integration | Catalog stock validation                | Order cancelled                           | `src/Catalog.API/IntegrationEvents/Events/OrderStockRejectedIntegrationEvent.cs`                 | 0.88       |
| 12  | Order Payment Succeeded Integration | Integration | PaymentProcessor success                | Order advances to Paid                    | `src/Ordering.API/Application/IntegrationEvents/Events/OrderPaymentSucceededIntegrationEvent.cs` | 0.88       |
| 13  | Order Payment Failed Integration    | Integration | PaymentProcessor failure                | Order cancelled                           | `src/Ordering.API/Application/IntegrationEvents/Events/OrderPaymentFailedIntegrationEvent .cs`   | 0.88       |
| 14  | Product Price Changed Integration   | Integration | Catalog item price update               | Webhook subscribers notified              | `src/Catalog.API/IntegrationEvents/Events/ProductPriceChangedIntegrationEvent.cs`                | 0.84       |
...
| Name                | Type              | Description                                                                                  | Source                                                                     | Confidence  | Maturity |
| ------------------- | ----------------- | -------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ----------- | -------- |
| Order               | Aggregate Root    | Purchase commitment with lifecycle, items, address, payment reference, and buyer association | `src/Ordering.Domain/AggregatesModel/OrderAggregate/Order.cs:1-160`        | 0.93 (HIGH) | Level 4  |
| OrderItem           | Entity            | Product line in an order with quantity, price, discount cap                                  | `src/Ordering.Domain/AggregatesModel/OrderAggregate/OrderItem.cs`          | 0.88 (HIGH) | Level 3  |
| OrderStatus         | Value Enumeration | Six-state business lifecycle enumeration                                                     | `src/Ordering.Domain/AggregatesModel/OrderAggregate/OrderStatus.cs:5-14`   | 0.91 (HIGH) | Level 4  |
| Buyer               | Aggregate Root    | Customer business identity with payment method registry                                      | `src/Ordering.Domain/AggregatesModel/BuyerAggregate/Buyer.cs:1-55`         | 0.88 (HIGH) | Level 3  |
| PaymentMethod       | Entity            | Registered payment instrument with card data and type                                        | `src/Ordering.Domain/AggregatesModel/BuyerAggregate/PaymentMethod.cs:1-45` | 0.86 (HIGH) | Level 3  |
| Address             | Value Object      | Delivery location with 5-field composition equality                                          | `src/Ordering.Domain/AggregatesModel/OrderAggregate/Address.cs:1-32`       | 0.87 (HIGH) | Level 3  |
| CatalogItem         | Entity            | Product with pricing, stock levels, brand, type, embeddings                                  | `src/Catalog.API/Model/CatalogItem.cs:1-62`                                | 0.86 (HIGH) | Level 3  |
| CustomerBasket      | Entity            | Buyer's pre-order item selection                                                             | `src/Basket.API/Model/CustomerBasket.cs:1-20`                              | 0.83 (HIGH) | Level 3  |
| WebhookSubscription | Entity            | Registered external notification endpoint by type and user                                   | `src/Webhooks.API/Model/WebhookSubscription.cs:1-15`                       | 0.81 (HIGH) | Level 3  |
| ApplicationUser     | Entity            | Full customer profile: contact, payment card, identity credentials                           | `src/Identity.API/Models/ApplicationUser.cs:1-30`                          | 0.82 (HIGH) | Level 3  |

### 5.11 KPIs & Metrics

| Name                              | Type                    | Description                                                                                                                    | Observable Indicator                                                                                          | Source                                                                                                                            | Confidence    | Maturity |
| --------------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------- | -------- |
| Order Status Distribution         | Operational KPI         | Count of orders in each of the six lifecycle states at any point in time, enabling backlog and flow visibility                 | OrderStatus enum values; orders queryable via IOrderQueries.GetOrdersFromUserAsync                            | `src/Ordering.Domain/AggregatesModel/OrderAggregate/OrderStatus.cs:5-14`, `src/Ordering.API/Application/Queries/IOrderQueries.cs` | 0.78 (MEDIUM) | Level 2  |
| Stock Level vs. Reorder Threshold | Inventory KPI           | Ratio of AvailableStock to RestockThreshold per catalog item; triggers reorder advisory when AvailableStock ≤ RestockThreshold | CatalogItem.AvailableStock, CatalogItem.RestockThreshold, CatalogItem.MaxStockThreshold                       | `src/Catalog.API/Model/CatalogItem.cs:25-35`                                                                                      | 0.77 (MEDIUM) | Level 2  |
| Grace Period Duration             | Process Timing KPI      | Elapsed time between order submission and grace period confirmation; configurable via BackgroundTaskOptions.CheckUpdateTime    | GracePeriodManagerService poll interval; CheckUpdateTime seconds                                              | `src/OrderProcessor/BackgroundTaskOptions.cs:1-10`                                                                                | 0.73 (MEDIUM) | Level 2  |
| Payment Outcome Rate              | Payment Performance KPI | Rate of payment success vs. failure across processed orders; currently configurable, not metered in real time                  | PaymentOptions.PaymentSucceeded flag; PaymentSucceededIntegrationEvent / PaymentFailedIntegrationEvent counts | `src/PaymentProcessor/PaymentOptions.cs:1-10`                                                                                     | 0.73 (MEDIUM) | Level 2  |
...
| --------------------------- | -------------------------- | ---------------------------------- | ---------------------------------------------------- | ------------------------- | ------------------------------ |
| Basket Clearance            | Basket Management          | Order Lifecycle Management         | Event Bus (OrderStartedIntegrationEvent)             | Ordering → Basket         | On order placement             |
...
| Grace Period Timer          | Order Lifecycle Management | Order Processor                    | Event Bus (GracePeriodConfirmedIntegrationEvent)     | OrderProcessor → Ordering | On timer interval              |
...
| Catalog Notification        | Event-Driven Notification  | Product Catalog Management         | Event Bus (ProductPriceChangedIntegrationEvent)      | Catalog → Webhooks        | On item price change           |
...
| Pattern                                 | Applied By                                   | Purpose                                                                            | Source Evidence                                                                   |
| --------------------------------------- | -------------------------------------------- | ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| Event-Driven Choreography               | Ordering, Catalog, Basket, Payment, Webhooks | Asynchronous cross-service state progression with temporal decoupling              | `src/Ordering.API/Application/IntegrationEvents/`, `src/EventBusRabbitMQ/`        |
| CQRS (Command / Query Separation)       | Ordering Service                             | Separate write (commands) and read (queries) models for order data                 | `src/Ordering.API/Application/Commands/`, `src/Ordering.API/Application/Queries/` |
| Idempotent Command Processing           | Ordering Service                             | Prevents duplicate order creation on network retries via request ID tracking       | `src/Ordering.API/Application/Commands/IdentifiedCommandHandler.cs`               |
| Domain Event → Integration Event Bridge | Ordering Service                             | Domain events are translated to integration events after transactional commit      | `src/Ordering.API/Application/Behaviors/TransactionBehavior.cs:1-60`              |
| Background Service Timer                | Order Processor                              | Periodic business process poll (grace period) decoupled from web request lifecycle | `src/OrderProcessor/Services/GracePeriodManagerService.cs:1-65`                   |
| Webhook Subscription Model              | Webhook Service                              | Extensible pub-sub for external consumers, validated on subscription creation      | `src/Webhooks.API/`, `src/Webhooks.API/Services/GrantUrlTesterService.cs`         |
| gRPC Synchronous Service                | Basket Service                               | Low-latency, strongly-typed basket access by WebApp at checkout                    | `src/Basket.API/Grpc/`, `src/WebApp/Services/BasketService.cs`                    |
